...
public class TestExec {
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        // 获取标准输出，从这个文件对象中读，就能把子进程的标准输出读出来！
        ...
        // 获取标准错误，从这个文件对象中读，就能把子进程的标准错误读出来
        ...
        // 父进程执行到 waitFor 的时候，就会阻塞，知道子进程执行完毕
        // （和 Thread.join 是非常相似的）
        // 这个退出码就表示子进程的执行结果是否 ok，如果子进程是代码执行完了正常退出，退出码就是 0
        // 如果子进程代码执行了一般异常退出（抛异常），此时返回的退出码就是 非0
